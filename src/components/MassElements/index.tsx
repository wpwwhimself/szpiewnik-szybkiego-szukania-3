...
export function MassElemSection({id, uneresable = false, children}: MassElemSectionProps){
  return(
    ...
    {!uneresable &&
      <div className={`${style.massElemEraser} flex-right`}>
        <Button onClick={() => MMod.eraseMassElem(id)}>&#10003;</Button>
        <Button onClick={() => MMod.prepareMassElemErase(id)}>–</Button>
      </div>}
      {children}
    </section>
  )
...
export function SongLyrics({title}: {title: string}){
  const lyrics = songs
    ...
    .replace(/\*\s*\n/g, `<span class="${style.chorus}">`)
    .replace(/_(.{1,5})_/g, '<u>$1</u>')
    .replace(/\d+\.\s*\n/g, match => {return "<li start="+match.substring(0, match.length - 2)+">"})
    .replace(/\n/g, "<br />");

  return(<ol className={style.lyrics} dangerouslySetInnerHTML={{ __html: lyrics ?? ""}} />);
}
...
export function Antiphon({call, resp}: {call: string, resp: string}){
  return(
    <div className={`flex-right center ${style.antyfona}`}>
      <div>{call}</div>
      <div>→</div>
...
export function OrdinariumProcessor({code, colorCode}: OrdinariumProcessorProps){
  ...
  switch(code.substring(1)){
    case "Kyrie":
      return(
        <>
          ...
          <h2>Akt pokutny</h2>
          <Alternative>
            <div className={style.alt_group}>
              <h4><strong>Kyrie</strong> zostaje</h4>
              <div className={style.alt_option}>
                <p className={style.ksiadz}>Spowiadam się Bogu Wszechmogącemu...</p>
              </div>
              <div className={style.alt_option}>
                <Antiphon
          ...
          <p className={style.ksiadz}>Niech się zmiłuje nad nami Bóg Wszechmogący i, odpuściwszy nam grzechy, doprowadzi nas do życia wiecznego...</p>
    ...
    case "Credo":
      return(
        <>
          <p className={style.ksiadz}>Złóżmy wyznanie wiary:</p>
          <h1>Credo</h1>
          {parts.map((part, i) =>
            <div className="flex-right center sheet-container" key={i}>
              <Notation notation={part.sheetMusic} />
    ...
    case "Sanctus":
      return(
        <>
          ...
          />
          <p className={style.ksiadz}>Zaprawdę godne to i sprawiedliwe... ...jednym głosem wołając:</p>
      ...
      return(
        <>
          <Antiphon
            ...
            resp="Amen"
          />
          <p className={style.ksiadz}>Nazywamy się dziećmi Bożymi i nimi jesteśmy, dlatego ośmielamy się mówić:</p>
          <h1>Pater Noster</h1>
          {parts.map((part, i) =>
            <div className="flex-right center sheet-container" key={i}>
              <Notation notation={part.sheetMusic} />
      ...
      return(
        <>
          <Antiphon
            ...
            resp="I z duchem Twoim"
          />
          <p className={style.ksiadz}>Przekażcie sobie znak pokoju:</p>
    ...
    case "Wedding":
      return(
        <>
          ...
          <p className={style.ksiadz}>Prośmy więc Ducha Świętego... ...Chrystusa i Kościoła:</p>
        </>
      )
    case "Transf":
      return(
